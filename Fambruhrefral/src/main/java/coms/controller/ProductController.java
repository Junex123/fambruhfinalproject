...
public class ProductController {
    ...
    public ResponseEntity<?> addNewProduct(@RequestParam("product") String productData,
        ...
        img1.setFilePath(FOLDER_PATH + mainImageFile.getOriginalFilename());
        ...
        // Check if the mainImage is added to the folder
        if (addedProduct.getMainImage() != null) {
            System.out.println("Main Image added to folder: " + addedProduct.getMainImage().getFilePath());
        } else {
            System.out.println("Main Image was not added to folder.");
        }
	...
	public ResponseEntity<?> getProductById(@PathVariable("id") Long id){
		...
		ProductImageMain img1 = new ProductImageMain();
		img1.setFilePath(product.getMainImage().getFilePath()); // Directly set the file path
		...
		ProductImageDetail img3 =  new ProductImageDetail();
		img3.setImageData(ImageUtil.decompressImage(product.getDetailImage().getImageData()));
		img3.setImgId(product.getDetailImage().getImgId());
		img3.setName(product.getDetailImage().getName());
		img3.setType(product.getDetailImage().getType());
		product.setDetailImage(img3);
	...
	public ResponseEntity<?> getAllProducts(){
		...
		allProducts.forEach(product -> {
			ProductImageMain img1 =  new ProductImageMain();
			img1.setFilePath(product.getMainImage().getFilePath());
			img1.setImgId(product.getMainImage().getImgId());
			img1.setName(product.getMainImage().getName());
			img1.setType(product.getMainImage().getType());
			product.setMainImage(img1);
